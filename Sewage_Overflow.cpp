...
// HomeKit Sewage Overflow Sensor
struct DEV_SewageOverflow : Service::LeakSensor {
  SpanCharacteristic *leakDetected;
  
  DEV_SewageOverflow() : Service::LeakSensor() {
    leakDetected = new Characteristic::LeakDetected(0);
  }
  
  void loop() {
    int sewageLevel = Get_Water_level();
...
<!DOCTYPE HTML><html>
<head>
  <title>Sewage Level Monitor</title>
...
void setup() {
  ...
  homeSpan.begin(Category::Bridges, "Sewage Monitor");
